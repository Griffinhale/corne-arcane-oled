...
static uint8_t category_kind(uint8_t category) {
    switch (category) {
        case DUEL_HOST_CATEGORY_COMMUNICATION:
            return DUEL_CIVIC_COURIER_MESSENGER;
        case DUEL_HOST_CATEGORY_CALENDAR:
            return DUEL_CIVIC_COURIER_MESSENGER;
        case DUEL_HOST_CATEGORY_TRANSFER:
            return DUEL_CIVIC_COURIER_PARCEL;
        case DUEL_HOST_CATEGORY_SYSTEM:
            return DUEL_CIVIC_COURIER_BEACON;
        case DUEL_HOST_CATEGORY_TERMINAL:
            return DUEL_CIVIC_COURIER_BEACON;
        case DUEL_HOST_CATEGORY_SECURITY:
            return DUEL_CIVIC_COURIER_SENTINEL;
        case DUEL_HOST_CATEGORY_OTHER:
            return DUEL_CIVIC_COURIER_MESSENGER;
        default:
            return DUEL_CIVIC_COURIER_NONE;
    }
}
...
static uint8_t age_lifecycle(uint8_t age) {
    if (age == 0u)
        return DUEL_CIVIC_VISIT_ARRIVING; // new
    if (age <= 2u)
        return DUEL_CIVIC_VISIT_WAITING; // pending
    if (age <= 6u)
        return DUEL_CIVIC_VISIT_AGING; // old
    return DUEL_CIVIC_VISIT_RESOLVING; // aged to dismissal (7)
}

static uint8_t count_density(uint8_t count) {
    if (count >= 5u)
        return DUEL_CIVIC_DENSITY_MANY;
    if (count >= 2u)
        return DUEL_CIVIC_DENSITY_FEW;
...
uint8_t civic_visitor_shared_pres(civic_visitor_state_t state) {
    return (uint8_t)(DUEL_VISITOR_PACK(DUEL_VISITOR_STATE_KIND(state),
                                       DUEL_VISITOR_STATE_CITY(state), state.lifecycle_phase) |
                     DUEL_VISITOR_DENSITY_PACK(DUEL_VISITOR_STATE_DENSITY(state)));
}

static void draw_courier_floor_mark(duel_fb_t *fb, bool is_left, uint8_t kind, uint8_t floor, int x,
                                    int y) {
    ...
    static const int8_t mark[4][3][3][2] = {
        {{{-1, 2}, {0, 2}, {1, 2}}, {{-1, 2}, {0, 3}, {1, 2}}, {{-1, 2}, {1, 2}, {0, 1}}},
        {{{0, -4}, {0, -3}, {0, -2}}, {{-1, -5}, {0, -5}, {1, -5}}, {{-2, 0}, {2, -4}, {0, -2}}},
        {{{-2, -6}, {-1, -7}, {1, -7}},
         {{-1, -8}, {1, -8}, {2, -6}},
         {{-1, -7}, {1, -7}, {-1, -4}}},
        {{{-2, -7}, {-1, -7}, {0, -7}},
...
void draw_courier(duel_fb_t *fb, const duel_render_t *r, bool is_left) {
    ...
    incantation_point_t destination =
        incantation_occupation_anchor(floor, destination_action[kind]);
...
#define INCANTATION_LIFT_Y 72
    ...
    switch (kind) {
        case DUEL_CIVIC_COURIER_MESSENGER:
            ...
            duel_fb_px(fb, cx - 1, y - 1, true);
            duel_fb_px(fb, cx + 1, y - 1, true);
            duel_fb_px(fb, cx - 2, y, true);
            duel_fb_px(fb, cx + 2, y, true);
    ...
    if (density >= DUEL_CIVIC_DENSITY_FEW) {
        duel_fb_px(fb, cx - 3, y, true);
        duel_fb_px(fb, cx + 3, y, true);
    }
    if (density >= DUEL_CIVIC_DENSITY_MANY) {
        duel_fb_px(fb, cx - 2, y + 4, true);
        duel_fb_px(fb, cx + 2, y + 4, true);
    }
    if (life == DUEL_CIVIC_VISIT_ARRIVING && !quiet) {
        duel_fb_px(fb, cx - 2 * g, y + 2, true);
        duel_fb_px(fb, cx - 4 * g, y + 3, true);
    } else if (life == DUEL_CIVIC_VISIT_WAITING) {
        duel_fb_px(fb, cx, y + 3, true);
    } else if (life == DUEL_CIVIC_VISIT_AGING) {
        duel_fb_px(fb, cx - 1, y + 3, true);
        duel_fb_px(fb, cx + 1, y + 4, true);
    } else if (life == DUEL_CIVIC_VISIT_RESOLVING && !quiet) {
        duel_fb_px(fb, cx + 3 * g, y, true);
        duel_fb_px(fb, cx + 4 * g, y - 1, true);
    }
}
