"""Fail-soft, privacy-bounded semantic adapters for desktop system state."""
...
class SemanticAdapters:
    """Pure state transitions used by the D-Bus monitors and unit tests."""
    ...
    def pomodoro(self, active: bool, remaining_seconds: float | None = None, failed: bool = False) -> None:
        now = self.clock()
    ...
    def repository(self, state: int, success: bool) -> bool:
        priorities = {
            0: Priority.LOW,      # clean
            1: Priority.NORMAL,   # dirty
            2: Priority.NORMAL,   # operation
            3: Priority.NORMAL if success else Priority.CRITICAL,  # completion
        }
        priority = priorities.get(state)
        if priority is None:
            return False
