...
uint8_t duel_scry_mask_from_rows(const uint16_t rows[DUEL_INPUT_ROWS]) {
    uint8_t mask = 0;
    if (rows[SCRY_KEY_L_ROW] & (1u << SCRY_KEY_L_COL))
        mask |= SCRY_M_L;
    if (rows[SCRY_KEY_R_ROW] & (1u << SCRY_KEY_R_COL))
        mask |= SCRY_M_R;
    ...
    for (uint8_t r = 0; r < DUEL_INPUT_ROWS; r++) {
        uint16_t row = rows[r];
        if (r == SCRY_KEY_L_ROW)
            row &= (uint16_t)~(1u << SCRY_KEY_L_COL);
        if (r == SCRY_KEY_R_ROW)
            row &= (uint16_t)~(1u << SCRY_KEY_R_COL);
        if (row) {
            mask |= SCRY_M_OTHER;
            break;
        }
...
bool duel_mailbox_consume(const duel_mailbox_t *mailbox, uint8_t *seen_version, void *destination,
    ...
    if (first == *seen_version || (first & 1u))
        return false;
    DUEL_COMPILER_BARRIER();
...
bool duel_tx_attempt(duel_tx_policy_t *policy, uint32_t now_ms, bool urgent, bool fx_changed,
    ...
    if (elapsed < DUEL_ACTIVE_TX_MS)
        return false;
    if (urgent || fx_changed || semantic_changed)
        return true;
...
uint8_t duel_floor_presentation(duel_floor_policy_t *policy, uint32_t now_ms) {
    if (policy->active && now_ms - policy->started_ms >= DUEL_FLOOR_TRANSITION_MS)
        policy->active = false;
    uint8_t phase =
        policy->active ? (uint8_t)((now_ms - policy->started_ms) / DUEL_FLOOR_PHASE_MS) : 0u;
...
bool duel_flash_note(duel_flash_policy_t *policy, uint8_t fx_seq, uint8_t kind, uint8_t spell_kind,
    ...
    policy->started_ms = now_ms;
    policy->duration_ms = (kind == FX_IMPACT_L || kind == FX_IMPACT_R) ? DUEL_PRESENTATION_IMPACT_MS
...
bool duel_flash_observe_view(duel_flash_policy_t *policy, uint8_t last_spell_kind[2],
    ...
    return duel_flash_note(policy, fx_seq, flash_kind,
                           last_spell_kind[defender_left ? SIM_SIDE_R : SIM_SIDE_L], now_ms);
}
...
bool duel_display_should_follow(uint8_t remote_phase, uint32_t *wake_until_ms, uint32_t now_ms) {
    return remote_phase <= DUEL_DISPLAY_SLEEP && !duel_wake_grace_active(wake_until_ms, now_ms);
}

uint8_t duel_sky_phase(uint32_t session_elapsed_ms) {
    uint32_t within = session_elapsed_ms % DUEL_SKY_CYCLE_MS;
    if (within < 150000u)
        return DUEL_SKY_DAWN;
    if (within < 1350000u)
        return DUEL_SKY_DAY;
    if (within < 1500000u)
        return DUEL_SKY_DUSK;
...
uint8_t duel_sky_subphase(uint32_t session_elapsed_ms) {
    ...
    else if (within >= 1350000u) {
        start = 1350000u;
        length = 150000u;
    } /* dusk */
    else if (within >= 150000u) {
        start = 150000u;
        length = 1200000u;
    } /* day */
...
void duel_diplomacy_init(duel_diplomacy_t *state) { memset(state, 0, sizeof *state); }
...
duel_civic_shared_t duel_civic_shared_derive(uint8_t session, uint32_t now_ms,
    ...
    bool observatory = DUEL_CIVIC_FLOOR(duel_host_civic(host)) == DUEL_CIVIC_FLOOR_SPECIAL;
    ...
    out.revision = civic_event_revision(
        civic_event_derive(session, phase, eligible && !observatory, diplomacy_balance));
...
duel_slave_decision_t duel_slave_present(duel_slave_presenter_t *presenter, bool accepted,
    ...
    if (!stale && have_any) {
        ...
        out.base_refresh = accepted || !presenter->using_remote || stale_edge || render_invalid;
        presenter->using_remote = true;
    } else {
        // Local pose-only fallback: never authoritative, never combat.
        out.base_refresh = ticked || presenter->using_remote || stale_edge || render_invalid;
        out.set_stale = stale;
