...
static bool pixels_within(const duel_fb_t *fb, int y0, int y1) {
    for (int y = 0; y < DUEL_CANVAS_H; y++)
        for (int x = 0; x < DUEL_CANVAS_W; x++)
            if (duel_fb_get(fb, x, y) && (y < y0 || y > y1))
                return false;
    return true;
}
...
static void test_civic_anchor_and_courier_matrix(void) {
    ...
    for (uint8_t action = 0; action < DUEL_CIVIC_ACTION_COUNT; action++) {
        incantation_point_t fallback =
            incantation_occupation_anchor(INCANTATION_OCCUPATION_FLOORS, action);
        incantation_point_t commons =
            incantation_occupation_anchor(DUEL_CIVIC_FLOOR_COMMONS, action);
        ...
        for (uint8_t floor = 0; floor < INCANTATION_OCCUPATION_FLOORS; floor++) {
            ...
            EXPECT(point.x >= 0 && point.x < DUEL_CANVAS_W && point.y >= DUEL_FLOOR_BEAM_Y &&
                   point.y <= DUEL_FLOOR_Y1);
        }
    }
        ...
        for (uint8_t kind = DUEL_CIVIC_COURIER_MESSENGER; kind < DUEL_CIVIC_COURIER_COUNT; kind++)
            for (uint8_t life = DUEL_CIVIC_VISIT_ARRIVING; life <= DUEL_CIVIC_VISIT_RESOLVING;
                ...
                for (uint8_t density = DUEL_CIVIC_DENSITY_SINGLE;
                    ...
                    for (uint8_t mode = DUEL_CIVIC_MODE_NORMAL; mode <= DUEL_CIVIC_MODE_QUIET;
                         mode++)
                        for (uint8_t city = 0; city < 2u; city++) {
                            duel_render_t r = {0};
                            duel_render_from_world(&r, &world);
                            ...
                            duel_fb_clear(&assigned);
                            duel_fb_clear(&repeat);
                            duel_fb_clear(&opposite);
    ...
    duel_render_t route = {0};
    duel_render_from_world(&route, &world);
    route.civic = DUEL_CIVIC_PACK(DUEL_CIVIC_FLOOR_WORKSHOP, DUEL_CIVIC_MODE_NORMAL, 0);
    route.shared_pres = DUEL_VISITOR_PACK(DUEL_CIVIC_COURIER_PARCEL, 0, DUEL_CIVIC_VISIT_WAITING);
    route.floor_transition = INCANTATION_FLOOR_TRANSITION_PACK(DUEL_CIVIC_FLOOR_COMMONS, 1, true);
    duel_fb_t source, expected, mirror;
    duel_fb_clear(&source);
    duel_fb_clear(&expected);
    duel_fb_clear(&mirror);
    ...
    route.shared_pres = DUEL_VISITOR_PACK(DUEL_CIVIC_COURIER_PARCEL, 1, DUEL_CIVIC_VISIT_WAITING);
    ...
    for (uint8_t kind = DUEL_CIVIC_COURIER_MESSENGER; kind < DUEL_CIVIC_COURIER_COUNT; kind++) {
        ...
        for (uint8_t floor = 0; floor < INCANTATION_OCCUPATION_FLOORS; floor++) {
            ...
            draw_courier(&variant[floor], &route, true);
        }
...
static void test_rare_event_floor_phase_mode_target_matrix(void) {
    ...
    for (uint8_t floor = 0; floor < INCANTATION_OCCUPATION_FLOORS; floor++)
        for (uint8_t id = DUEL_CIVIC_EVENT_RUNAWAY_SCROLL; id < DUEL_CIVIC_EVENT_COUNT; id++)
            for (uint8_t phase = DUEL_CIVIC_EVENT_PHASE_ARMED;
                ...
                for (uint8_t mode = DUEL_CIVIC_MODE_NORMAL; mode <= DUEL_CIVIC_MODE_QUIET; mode++) {
                    ...
                    for (uint8_t target_case = 0; target_case < (shared ? 1u : 2u); target_case++) {
                        ...
                        if (floor == DUEL_CIVIC_FLOOR_SPECIAL) {
                            ...
                            EXPECT(framebuffer_pixels(&left) >= 4u &&
                                   framebuffer_pixels(&right) >= 4u);
                        } else if (target == DUEL_CIVIC_EVENT_TARGET_LEFT) {
                            EXPECT(framebuffer_pixels(&left) >= 4u &&
                                   framebuffer_pixels(&right) == 0u);
                        } else {
                            EXPECT(framebuffer_pixels(&right) >= 4u &&
                                   framebuffer_pixels(&left) == 0u);
                        }
    ...
    for (uint8_t id = DUEL_CIVIC_EVENT_RUNAWAY_SCROLL; id < DUEL_CIVIC_EVENT_COUNT; id++) {
        ...
        for (uint8_t floor = 0; floor < INCANTATION_OCCUPATION_FLOORS; floor++) {
            ...
            duel_fb_clear(&variant[floor]);
            draw_rare_event(&variant[floor], &none, true);
        }
        EXPECT(memcmp(&variant[0], &variant[1], sizeof variant[0]) != 0 &&
               memcmp(&variant[0], &variant[2], sizeof variant[0]) != 0 &&
               memcmp(&variant[1], &variant[2], sizeof variant[0]) != 0);
        ...
        for (uint8_t kind = AFTER_CHEER; kind <= AFTER_MAX_CAST; kind++)
            for (uint8_t phase = 0; phase < 4u; phase++)
                for (uint8_t side = 0; side < 2u; side++) {
                    ...
                    base.seed = 0x51u;
                    base.civic_phase = 23u;
                    base.civic = DUEL_CIVIC_PACK(floor, DUEL_CIVIC_MODE_NORMAL, 0);
                    duel_render_t after = base;
                    after.shared_pres = (uint8_t)((kind << (side * 3u)) | (WORLD_RECOVERY << 6));
                    after.revision = (uint8_t)(INCANTATION_AFTERMATH_WIRE | (phase << (side * 2u)));
                    ...
                    EXPECT(memcmp(&first, &second, sizeof first) == 0 &&
                           ...
                           band_difference(&before, &first, DUEL_FLOOR_Y1 + 1, DUEL_CANVAS_H - 1) ==
                               0u);
                }
    ...
    for (uint16_t seed = 0; seed < 256u; seed++) {
        for (uint8_t side = 0; side < 2u; side++) {
            ...
            for (uint8_t slot = 0; slot < 16u; slot++) {
                ...
                civic_resident_t common =
                    civic_resident_derive((uint8_t)seed, side == 0u, DUEL_CIVIC_FLOOR_COMMONS,
                                          DUEL_CIVIC_MODE_NORMAL, phase);
                for (uint8_t floor = 0; floor < INCANTATION_OCCUPATION_FLOORS; floor++) {
                    civic_resident_t a = civic_resident_derive((uint8_t)seed, side == 0u, floor,
                                                               DUEL_CIVIC_MODE_NORMAL, phase);
                    civic_resident_t b = civic_resident_derive((uint8_t)seed, side == 0u, floor,
                                                               DUEL_CIVIC_MODE_NORMAL, phase);
                    ...
                    civic_resident_t quiet = civic_resident_derive((uint8_t)seed, side == 0u, floor,
                                                                   DUEL_CIVIC_MODE_QUIET, phase);
                    EXPECT(quiet.action == quiet_action(common.action) &&
                           quiet.station == INCANTATION_OCCUPATION_KEY(floor, quiet.action));
                }
                civic_resident_t special =
                    civic_resident_derive((uint8_t)seed, side == 0u, DUEL_CIVIC_FLOOR_SPECIAL,
                                          DUEL_CIVIC_MODE_NORMAL, phase);
                EXPECT(special.action == common.action &&
                       special.station ==
                           INCANTATION_OCCUPATION_KEY(DUEL_CIVIC_FLOOR_SPECIAL, special.action));
            }
        }
    }
    for (uint8_t side = 0; side < 2u; side++) {
        for (uint8_t p = 0; p < DUEL_CIVIC_PERSONALITY_COUNT; p++)
...
static void test_resident_geometry_and_object_separation(void) {
    bool ok = true;
    for (uint8_t side = 0; side < 2u; side++) {
        civic_resident_t core = {
            DUEL_CIVIC_PERSONALITY_DILIGENT, DUEL_CIVIC_ACTION_WORK,
