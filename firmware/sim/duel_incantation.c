...
static uint8_t sat_inc(uint8_t value) { return value == 0xffu ? value : (uint8_t)(value + 1u); }

static uint8_t gap_bucket(uint8_t ticks) {
    return ticks <= 1u   ? TEMPO_FRANTIC
           : ticks <= 2u ? TEMPO_RAPID
           : ticks <= 4u ? TEMPO_FLOWING
                         : TEMPO_DELIBERATE;
}

static void hash_byte(uint32_t *hash, uint8_t value) { *hash = (*hash ^ value) * FNV1A_PRIME; }

static void hash_token(sim_incantation_t *incantation, bool down, uint8_t position, uint8_t layer,
                       uint8_t quantum) {
    hash_byte(&incantation->hash, (uint8_t)((down ? 0x80u : 0u) | (position & 0x1fu)));
    hash_byte(&incantation->hash, (uint8_t)(((layer & 3u) << 4) | gap_bucket(quantum)));
}
...
uint8_t incantation_collection_keydown(sim_incantation_t *incantation, uint8_t position,
    ...
    if (incantation->key_count) {
        ...
        if (direction && incantation->last_direction && direction != incantation->last_direction)
            incantation->turns = sat_inc(incantation->turns);
        if (direction)
            incantation->last_direction = direction;
        ...
        if (!incantation->gap_count)
            incantation->first_gap = gap;
        ...
        if (gap < incantation->gap_min)
            incantation->gap_min = gap;
        if (gap > incantation->gap_max)
            incantation->gap_max = gap;
    }
...
void incantation_collection_keyup(sim_incantation_t *incantation, uint8_t position) {
    hash_token(incantation, false, position, incantation->last_layer, incantation->idle);
}
...
void incantation_collection_tick(sim_incantation_t *incantation, uint32_t held_positions) {
    ...
    if (overlap > incantation->overlap_peak)
        incantation->overlap_peak = overlap;
}
...
static uint8_t dominant_row(const sim_incantation_t *inc, uint8_t affinity_row) {
    ...
    for (uint8_t row = 0; row < 4; row++) {
        ...
        if (n > best_n || (n == best_n && best != affinity_row &&
...
static uint8_t choose_form(uint8_t complexity, uint8_t variant, uint8_t temper, uint32_t hash) {
    ...
    for (uint8_t i = 0; i < eligible; i++) {
        bool preferred =
            (variant == 0u && (forms[i] == SPELL_PROJECTILE || forms[i] == SPELL_SWARM)) ||
...
uint8_t incantation_tempo_trend(const sim_incantation_t *inc) {
    ...
    if (inc->gap_count) {
        ...
        tempo = avg <= 1u   ? TEMPO_FRANTIC
                : avg <= 2u ? TEMPO_RAPID
                : avg <= 4u ? TEMPO_FLOWING
                            : TEMPO_DELIBERATE;
    }
    uint8_t trend = TREND_STEADY;
    if (inc->gap_count > 1u) {
        if ((uint8_t)(inc->gap_max - inc->gap_min) >= 4u)
            trend = TREND_IRREGULAR;
        else if (inc->last_gap < inc->first_gap)
...
uint8_t incantation_local_ambience(const sim_wizard_t *wizard) {
    ...
    if (wizard->inc_state == INC_WINDUP || wizard->inc_state == INC_PREPARED) {
        uint32_t desc =
            wizard->inc_state == INC_PREPARED ? wizard->prepared_desc : wizard->pending_desc;
        if (SPELL_DESC_VALID(desc))
            return INCANTATION_AMBIENCE_PACK(true, SPELL_DESC_TEMPO(desc), SPELL_DESC_TREND(desc));
    }
    return 0;
}
...
uint32_t incantation_compile(const sim_incantation_t *inc, uint8_t variant, uint8_t temper) {
    ...
    uint8_t magnitude = complexity < 48u    ? 1u
                        : complexity < 112u ? 2u
                        : complexity < 192u ? 3u
                                            : 4u;
    ...
    uint8_t payload;
    if (inc->layer_transitions >= 3u)
        payload = PAY_HYBRID;
    else if (held_dense || inc->overlap_peak >= 3u)
        payload = PAY_STATUS;
    else if (strong_inward || ((inc->hash >> 17) & 7u) == 0u)
        payload = PAY_HEAL;
    else
        payload = PAY_DAMAGE;
    ...
    uint8_t interaction = form == SPELL_SINGULARITY      ? INTERACT_ABSORB
                          ...
                          : inc->layer_transitions >= 6u ? INTERACT_COMBINE
                                                         : INTERACT_SOLID;
    ...
    if (payload == PAY_STATUS || payload == PAY_HYBRID) {
        status = element == ELEM_EMBER   ? STATUS_BURNING
                 ...
                 : element == ELEM_VOID  ? STATUS_DISRUPTED
                                         : STATUS_MARKED;
    }
    ...
    return SPELL_DESC_PACK(form, element, payload, trajectory, magnitude, status, interaction,
                           tempo, trend, (inc->hash >> 22) & 3u);
}
