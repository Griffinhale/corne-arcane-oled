    ...
    if (condition) printf("PASS %s\n", name); \
        ...
        if (test_expect_expr) \
            printf("     first failing sub-assertion (line %d): %s\n", \
                   test_expect_line, test_expect_expr); \
        test_failures++; \
    } \
...
void step(sim_world_t *world, uint32_t left, uint32_t right,
          uint8_t left_layer, uint8_t right_layer,
...
void install_spell(sim_world_t *world, uint8_t side, uint32_t descriptor,
                   uint8_t progress);
...
unsigned band_difference(const duel_fb_t *left, const duel_fb_t *right,
                         int y0, int y1);
unsigned framebuffer_pixels(const duel_fb_t *framebuffer);
void incantation_render(duel_fb_t *framebuffer, const duel_render_t *render,
                        bool is_left, bool diagnostics);
void render_floor_scene(uint8_t floor, bool is_left, uint8_t transition,
