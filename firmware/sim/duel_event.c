...
static uint8_t event_weight(uint8_t family, int8_t balance) {
    ...
    return family == DUEL_CIVIC_EVENT_DIPLOMATIC_COURIER ? (uint8_t)(4u + 2u * magnitude)
                                                         : event_weights[family];
}
    ...
    for (uint16_t c = 0; c <= cycle; c++) {
        ...
        if (fam == prev) {
            ...
            while (fam == prev && guard < DUEL_CIVIC_EVENT_COUNT) {
                fam = (uint8_t)(fam + 1u);
                if (fam >= DUEL_CIVIC_EVENT_COUNT)
                    fam = DUEL_CIVIC_EVENT_RUNAWAY_SCROLL;
                guard++;
            }
        }
        prev = fam;
...
static uint8_t event_target_for(uint8_t family, uint8_t seed, uint8_t cycle, int8_t balance) {
...
static void draw_floor_event(duel_fb_t *fb, bool is_left, uint8_t floor, uint8_t id, uint8_t phase,
    ...
    switch (id) {
        ...
        case DUEL_CIVIC_EVENT_JAMMED_GEAR:
            ...
            event_px(fb, is_left, x - 3, y);
            event_px(fb, is_left, x + 3, y);
            event_px(fb, is_left, x, y - 3);
            event_px(fb, is_left, x, y + 3);
    ...
    if (id == DUEL_CIVIC_EVENT_DIPLOMATIC_COURIER) {
        ...
        for (int x = 0; x < DUEL_CANVAS_W; x++) {
            ...
            int dy = floor == DUEL_CIVIC_FLOOR_COMMONS    ? ((x / 4) & 1)
                     : floor == DUEL_CIVIC_FLOOR_RESEARCH ? ((x / 3) & 1)
                                                          : ((x / 2) & 1);
            duel_fb_px(fb, x, base - dy, true);
        }
        int streamers = phase == DUEL_CIVIC_EVENT_PHASE_ACTIVE     ? 3
                        : phase == DUEL_CIVIC_EVENT_PHASE_COOLDOWN ? 0
                                                                   : 1;
        for (int i = 0; i < streamers; i++) {
            int sx = (at.x + i * 7) & 31;
            if (sx < DUEL_TOWER_W)
                continue; /* desk-authored: tower is x0..12 */
