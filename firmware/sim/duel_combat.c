...
void duel_combat_ingest_events(sim_world_t *w, sim_inputs_t in,
                               const sim_event_t *ev, uint8_t n,
...
void duel_combat_collect_side(sim_world_t *w, sim_inputs_t in, uint8_t side,
                              uint32_t event_down) {
    sim_wizard_t *wz = &w->wiz[side];
...
void duel_combat_regeneration_step(sim_world_t *w) {
    for (uint8_t side = 0; side < 2; side++) {
        sim_wizard_t *wz = &w->wiz[side];
        if (wz->life != LIFE_ACTIVE || wz->hp >= SIM_MAX_HP || !wz->regen_ticks) continue;
