    ...
    switch (variant & 3) {
        case 1: // hat band: a cleared 1-px stripe across the widest hat row
            for (int x = cx - 2; x <= cx + 2; x++)
                duel_fb_px(fb, x, brim_y - 1, false);
            break;
...
void duel_combat_draw_downed(duel_fb_t *fb, int facing, uint8_t variant, int xo) {
    ...
    duel_fb_hline(fb, hx < head ? hx - 1 : head + 1, hx < head ? head - 1 : hx + 1,
                  72 + DUEL_ROOF_DY);
    duel_fb_px(fb, hx, 71 + DUEL_ROOF_DY, true);
    if ((variant & 3) == 3)
        duel_fb_px(fb, hx, 70 + DUEL_ROOF_DY, true); // pompom stayed on
...
void duel_combat_draw_medic(duel_fb_t *fb, int x, int facing) {
    ...
    duel_fb_px(fb, x + 1, 66 + DUEL_ROOF_DY, true);
    duel_fb_px(fb, x, 67 + DUEL_ROOF_DY, true);
    duel_fb_px(fb, x + 1, 67 + DUEL_ROOF_DY, true);
    for (int y = 68 + DUEL_ROOF_DY; y <= 70 + DUEL_ROOF_DY; y++)
...
#define BX(x) duel_fb_desk_x(is_left, x)
    if (posture == DUEL_BALCONY_BIGCAST) {
        ...
        duel_fb_px(fb, BX(14), 21, true); // head
        duel_fb_px(fb, BX(13), 22, true);
        duel_fb_px(fb, BX(14), 22, true);
        ...
        duel_fb_px(fb, BX(12), 21, true);
        duel_fb_px(fb, BX(15), 21, true);
        duel_fb_px(fb, BX(12), 20, true);
        duel_fb_px(fb, BX(15), 20, true);
        // Staff thrust up the gap-side of the shaft toward the finial, tip
        // sparking (render-frame cosmetic).
        for (int y = 14; y <= 20; y++)
            duel_fb_px(fb, BX(12), y, true);
        duel_fb_px(fb, BX(11), 15, true);
        duel_fb_px(fb, BX(13), 15, true);                            // orb head
        ...
        duel_fb_px(fb, BX(13), 23, true);
        duel_fb_px(fb, BX(14), 23, true);
        ...
        duel_fb_px(fb, BX(15 + (int)((frame >> 4) & 1u)), 23 - (int)((frame >> 3) & 1u), true);
    }
#undef BX
}
...
int duel_combat_spell_lane_y(uint8_t kind) {
    switch (DUEL_KIND_ELEMENT(kind)) {
        case ELEM_FROST:
            return SPELL_Y_BASE - 5;
        case ELEM_VOID:
            return SPELL_Y_BASE - 2;
        case ELEM_EMBER:
            return SPELL_Y_BASE + 3;
        default:
            return SPELL_Y_BASE;
    }
}
...
static void spell_glyph(duel_fb_t *fb, int x, int y, uint8_t kind, int dir, bool lift) {
    ...
    if (lift && tier < SPELL_TIER_SATURATED)
        tier++;
...
static int incantation_trajectory_y(uint32_t desc, uint8_t flight) {
    switch (SPELL_DESC_TRAJECTORY(desc)) {
        case TRAJ_GROUND:
            return SPELL_Y_BASE + 11;
        case TRAJ_LOW:
            return SPELL_Y_BASE + 5;
        case TRAJ_HIGH:
            return SPELL_Y_BASE - 7;
        case TRAJ_ROOF: {
            uint8_t half = flight < 128u ? flight : (uint8_t)(255u - flight);
            return SPELL_Y_BASE - 8 - half / 12;
        }
        case TRAJ_RETURNING:
            return SPELL_Y_BASE - 2 - flight / 24;
        case TRAJ_HOMING:
            return SPELL_Y_BASE - 7 + flight / 24;
        case TRAJ_AREA:
            return SPELL_Y_BASE;
        default:
            return SPELL_Y_BASE;
...
static void draw_orbiting_motes(duel_fb_t *fb, uint8_t count, int cx, int cy, uint32_t frame,
                                uint8_t spread, int facing) {
...
static uint8_t draw_tempo_interval(uint32_t desc, uint8_t deliberate, uint8_t flowing,
                                   uint8_t rapid, uint8_t frantic) {
...
static bool incantation_in_gap(uint8_t u) { return u >= DUEL_U_GAP_LO && u <= DUEL_U_GAP_HI; }

static void incantation_draw_inner_flare(duel_fb_t *fb, uint32_t desc, bool is_left, int y,
                                         uint8_t phase, uint8_t reach) {
...
static bool incantation_draw_gap_cue(duel_fb_t *fb, uint32_t desc, uint8_t form, uint8_t progress,
                                     uint8_t flight, uint8_t u, uint8_t caster_side, bool is_left,
                                     int y) {
    bool reflected_singularity = form == SPELL_SINGULARITY && progress >= 160u && progress <= 207u;
    if (!incantation_in_gap(u) && !reflected_singularity)
        return false;
    ...
    bool portal = SPELL_DESC_INTERACTION(desc) == INTERACT_PHASE ||
                  ...
                  SPELL_DESC_TRAJECTORY(desc) == TRAJ_RETURNING || form == SPELL_CONJURE ||
                  reflected_singularity;
    bool trail = form == SPELL_SWARM || SPELL_DESC_TRAJECTORY(desc) == TRAJ_HOMING ||
    ...
    if (portal) {
        ...
        if ((departure && !caster_local) || (!departure && caster_local))
            return true;
        uint8_t local = departure ? (uint8_t)(flight - DUEL_U_GAP_LO) : (uint8_t)(flight - 128u);
        uint8_t reach = departure ? (uint8_t)(3u - local / 11u) : (uint8_t)(1u + local / 11u);
        if (reach < 1u)
            reach = 1u;
        if (reach > 3u)
            reach = 3u;
...
void duel_combat_draw_spell(duel_fb_t *fb, const duel_view_spell_t *spell, uint8_t caster_side,
    ...
    if (form == SPELL_SWARM) {
        ...
        flight = phase < 12u
                     ? 8u
                     : draw_trend_flight(spell->descriptor, (uint8_t)((uint16_t)(phase - 12u) *
                                                                      240u / (interval - 1u)));
    }
    if (form == SPELL_CONJURE) {
        ...
        flight = trap ? (uint8_t)(phase < 16u ? phase * 5u : 80u)
                 : phase < 10u
                     ? 8u
                     : draw_trend_flight(spell->descriptor, (uint8_t)((uint16_t)(phase - 10u) *
                                                                      240u / (interval - 1u)));
    }
    if (SPELL_DESC_TRAJECTORY(spell->descriptor) == TRAJ_RETURNING && form != SPELL_CONJURE) {
        flight = flight < 128u ? flight : (uint8_t)(255u - flight);
        if (progress >= 128u)
            travel_dir = -travel_dir;
    }
    if (form == SPELL_SINGULARITY && progress < 192u)
        flight = 48u;
    uint8_t u = caster_side == SIM_SIDE_L ? flight : (uint8_t)(255u - flight);
    int x = 0;
    int bob = SPELL_DESC_TREND(spell->descriptor) == TREND_IRREGULAR
                  ? (int)((frame + SPELL_DESC_VARIANCE(spell->descriptor)) & 1u)
                  : 0;
    ...
    if (form == SPELL_BEAM) {
        ...
        if (full && SPELL_DESC_MAGNITUDE(spell->descriptor) >= 3u)
            duel_fb_line(fb, x0, yb - 1, x1, yb - 1);
        if (variant == 2u) {
            for (int px = 3; px < DUEL_CANVAS_W; px += 6)
                duel_fb_px(fb, px, yb - 2, true);
        }
        if (caster_local) {
    ...
    if (incantation_draw_gap_cue(fb, spell->descriptor, form, progress, flight, u, caster_side,
                                 is_left, y))
        return;
    ...
    if (form == SPELL_SWARM) {
        ...
        if (caster_local) {
            ...
            draw_orbiting_motes(fb, orbit_count, local_cx + facing * 2, SPELL_Y_BASE - 6, frame, 3u,
                                facing);
        }
        if (phase < 12u || !count)
            return;
...
void duel_combat_draw_charge(duel_fb_t *fb, const duel_view_wizard_t *wz, int facing,
    ...
    duel_fb_px(fb, cx, cy, true);
    if (stage >= 1) {
        duel_fb_px(fb, cx - radius, cy, true);
        duel_fb_px(fb, cx + radius, cy, true);
        duel_fb_px(fb, cx, cy - radius, true);
        duel_fb_px(fb, cx, cy + radius, true);
    }
    if (stage >= 2) {
        int d = radius > 1 ? radius - 1 : 1;
        duel_fb_px(fb, cx - d, cy - d, true);
        duel_fb_px(fb, cx + d, cy - d, true);
        duel_fb_px(fb, cx - d, cy + d, true);
        duel_fb_px(fb, cx + d, cy + d, true);
    }
    if (stage >= 3) {
        for (int d = -radius; d <= radius; d++) {
            if ((d & 1) == 0)
                duel_fb_px(fb, cx + d, cy - radius - 2, true);
        }
    }
    if (stage >= 4) {
...
void duel_combat_draw_ward(duel_fb_t *fb, int facing, int strength, int focus, bool punctured,
                           int puncture_y) {
    int ax = 16 + facing * 9;
    int focus_y = focus == 0   ? SPELL_Y_BASE + 10
                  : focus == 1 ? SPELL_Y_BASE + 5
                  : focus == 3 ? SPELL_Y_BASE - 7
                               : SPELL_Y_BASE;
    ...
    if (y1 + 1 < DUEL_DECK_Y0)
        duel_fb_px(fb, ax - facing, y1 + 1, true);
    if (y1 + 2 < DUEL_DECK_Y0)
