...
static void dump_case_pgm(const char *name, const duel_fb_t *left, const duel_fb_t *right) {
    char path[512];
    ...
    for (int y = 0; y < DUEL_CANVAS_H; y++) {
        for (int x = 0; x < width; x++) {
            int value;
            if (x < DUEL_CANVAS_W)
                value = duel_fb_get(left, x, y) ? 255 : 0;
            else if (x < DUEL_CANVAS_W + 3)
                value = 96;
            else
                value = duel_fb_get(right, x - DUEL_CANVAS_W - 3, y) ? 255 : 0;
 ...
 * capacity guard. */
static void record_case(const char *name, const duel_fb_t *left, const duel_fb_t *right) {
    ...
    if (dump_dir)
        dump_case_pgm(name, left, right);
}

static void record_render(const char *name, const duel_render_t *render, uint32_t frame, bool hud) {
    ...
    for (uint16_t seed = 0; seed < 256u && !found; seed++) {
        for (uint8_t slot = 0; slot < 16u; slot++) {
            ...
            civic_resident_t resident =
                civic_resident_derive((uint8_t)seed, is_left, floor, DUEL_CIVIC_MODE_NORMAL, phase);
...
static void add_render_case(const char *name, const duel_render_t *render, uint32_t frame) {
    record_render(name, render, frame, false);
}
...
static void add_bilateral_attunement_case(const char *name, sim_world_t *world) {
    ...
    left_render.civic = right_render.civic =
        DUEL_CIVIC_PACK(DUEL_CIVIC_FLOOR_WORKSHOP, DUEL_CIVIC_MODE_NORMAL, 0);
...
static uint32_t descriptor(uint8_t form, uint8_t magnitude) {
    ...
    uint8_t trajectory = form == SPELL_FIREBALL      ? TRAJ_ROOF
                         : form == SPELL_SINGULARITY ? TRAJ_AREA
                                                     : TRAJ_MID;
    if (form == SPELL_GROUND_WAVE)
        trajectory = TRAJ_GROUND;
    if (form == SPELL_CHAIN)
        trajectory = TRAJ_HOMING;
    if (form == SPELL_CONJURE)
        trajectory = TRAJ_RETURNING;
    uint8_t interaction = form == SPELL_SINGULARITY ? INTERACT_ABSORB : INTERACT_SOLID;
    return SPELL_DESC_PACK(form, element[form], PAY_DAMAGE, trajectory, magnitude, STATUS_NONE,
...
static void build_catalog(void) {
    ...
    for (uint8_t floor = 0; floor < 3u; floor++) {
        for (uint8_t action = 0; action < DUEL_CIVIC_ACTION_COUNT; action++) {
            ...
            snprintf(name, sizeof name, "occupation_mech_%s_%s", floor_name[floor],
                     action_name[action]);
    ...
    static const char *event_name[] = {"none",      "scroll",   "jam", "break",
                                       "complaint", "diplomat", "sky"};
    ...
    for (uint8_t floor = 0; floor < DUEL_CIVIC_FLOOR_SPECIAL; floor++) {
        for (uint8_t kind = DUEL_CIVIC_COURIER_MESSENGER; kind < DUEL_CIVIC_COURIER_COUNT; kind++) {
            ...
            char name[48];
            snprintf(name, sizeof name, "courier_%s_%s", floor_name[floor], courier_name[kind]);
        ...
        for (uint8_t id = DUEL_CIVIC_EVENT_RUNAWAY_SCROLL; id < DUEL_CIVIC_EVENT_COUNT; id++) {
            ...
            char name[48];
            if (floor == DUEL_CIVIC_FLOOR_COMMONS && id == DUEL_CIVIC_EVENT_DIPLOMATIC_COURIER)
    ...
    for (uint8_t phase = DUEL_SKY_DAWN; phase <= DUEL_SKY_NIGHT; phase++) {
        duel_render_t arc = {0};
        duel_render_from_world(&arc, &world);
        arc.seed = 0x5au;
        arc.civic_phase = 19u;
        arc.civic = DUEL_CIVIC_PACK(DUEL_CIVIC_FLOOR_COMMONS, DUEL_CIVIC_MODE_NORMAL, 0);
        arc.secondary =
            DUEL_SECONDARY_SKY_SUB_PACK(DUEL_SECONDARY_SKY_PACK(0, phase), arc_sub[phase]);
        char name[48];
        snprintf(name, sizeof name, "sky_arc_%s_sub%u", sky_name[phase], arc_sub[phase]);
    ...
    for (uint8_t elem = ELEM_FORCE; elem <= ELEM_VOID; elem++) {
        duel_render_t marks = {0};
        duel_render_from_world(&marks, &world);
        marks.seed = 0x5au;
        marks.civic_phase = 19u;
        marks.civic = DUEL_CIVIC_PACK(DUEL_CIVIC_FLOOR_COMMONS, DUEL_CIVIC_MODE_NORMAL, 0);
        marks.residue[0] = (uint8_t)(elem | (1u << 2) | (elem << 4) | (2u << 6));
        marks.residue[1] = (uint8_t)(elem | (3u << 2) | (elem << 4) | (2u << 6));
        char name[48];
        snprintf(name, sizeof name, "residue_%s_ramp", element_name[elem]);
        add_render_case(name, &marks, 7u);
    }
    duel_render_t history = {0};
    duel_render_from_world(&history, &world);
    history.seed = 0x5au;
    history.civic_phase = 19u;
    history.civic = DUEL_CIVIC_PACK(DUEL_CIVIC_FLOOR_COMMONS, DUEL_CIVIC_MODE_NORMAL, 0);
    ...
    static const uint8_t ambience_trend[] = {TREND_DECELERATING, TREND_STEADY, TREND_ACCELERATING,
                                             TREND_IRREGULAR};
    ...
    for (uint8_t tempo = TEMPO_DELIBERATE; tempo <= TEMPO_FRANTIC; tempo++) {
        duel_render_t ambience = {0};
        duel_render_from_world(&ambience, &world);
        ambience.seed = 0x5au;
        ambience.civic_phase = 19u;
        ambience.civic = DUEL_CIVIC_PACK(DUEL_CIVIC_FLOOR_WORKSHOP, DUEL_CIVIC_MODE_NORMAL, 0);
        ambience.local_ambience = INCANTATION_AMBIENCE_PACK(true, tempo, ambience_trend[tempo]);
        char name[48];
        snprintf(name, sizeof name, "typing_%s", tempo_name[tempo]);
        add_render_case(name, &ambience, 7u);
    }

    static const char *diplomacy_name[] = {"left_advantage", "right_advantage", "balance"};
    static const uint8_t diplomacy_target[] = {DUEL_CIVIC_EVENT_TARGET_LEFT,
                                               DUEL_CIVIC_EVENT_TARGET_RIGHT,
                                               DUEL_CIVIC_EVENT_TARGET_SHARED};
    for (size_t i = 0; i < 2u; i++) { /* balance is the ordinary event case above */
        duel_render_t diplomacy = {0};
        duel_render_from_world(&diplomacy, &world);
        diplomacy.seed = 0x5au;
        diplomacy.civic_phase = 19u;
        diplomacy.civic = DUEL_CIVIC_PACK(DUEL_CIVIC_FLOOR_COMMONS, DUEL_CIVIC_MODE_NORMAL, 0);
        diplomacy.revision = DUEL_EVENT_PACK(DUEL_CIVIC_EVENT_DIPLOMATIC_COURIER,
                                             DUEL_CIVIC_EVENT_PHASE_ACTIVE, diplomacy_target[i]);
        char name[48];
        snprintf(name, sizeof name, "diplomacy_%s", diplomacy_name[i]);
        add_render_case(name, &diplomacy, 7u);
    }

    duel_render_t variant = {0};
    ...
    for (uint8_t life = DUEL_CIVIC_VISIT_ARRIVING; life <= DUEL_CIVIC_VISIT_RESOLVING; life++) {
        if (life == DUEL_CIVIC_VISIT_WAITING)
            continue;
        char name[48];
        snprintf(name, sizeof name, "courier_lifecycle_%u", life);
    ...
    variant.shared_pres =
        (uint8_t)(DUEL_VISITOR_PACK(DUEL_CIVIC_COURIER_BEACON, 0, DUEL_CIVIC_VISIT_WAITING) |
                  DUEL_VISITOR_DENSITY_PACK(DUEL_CIVIC_DENSITY_FEW));
    ...
    variant.view.outcome_overlay |= 0x10u;
    ...
    for (uint8_t phase = DUEL_CIVIC_EVENT_PHASE_ARMED; phase <= DUEL_CIVIC_EVENT_PHASE_COOLDOWN;
        ...
        if (phase == DUEL_CIVIC_EVENT_PHASE_ACTIVE)
            continue;
        char name[48];
        snprintf(name, sizeof name, "event_lifecycle_%u", phase);
        variant.revision =
    ...
    if (!scenario || !duel_scenario_build(scenario, &projected) ||
        DUEL_VISITOR_KIND(projected.shared_pres) == DUEL_CIVIC_COURIER_NONE)
        abort();
    scenario = duel_scenario_find("event-scroll");
    if (!scenario || !duel_scenario_build(scenario, &projected) ||
        DUEL_EVENT_ID(projected.revision) == DUEL_CIVIC_EVENT_NONE)
        abort();
    ...
    duel_render_t cross = {0};
    duel_render_from_world(&cross, &world);
    cross.seed = 0x39u;
    cross.civic_phase = 31u;
    ...
    cross.shared_pres =
        (uint8_t)(DUEL_VISITOR_PACK(DUEL_CIVIC_COURIER_PARCEL, 0, DUEL_CIVIC_VISIT_AGING) |
                  DUEL_VISITOR_DENSITY_PACK(DUEL_CIVIC_DENSITY_MANY));
    ...
    for (uint8_t scene = 0; scene < SCRY_SCENES; scene++) {
        ...
        world.scry.scene = scene;
        duel_render_t scry = {0};
        duel_render_from_world(&scry, &world);
        scry.seed = 0x5au;
        scry.civic_phase = 23u;
    ...
    for (uint8_t phase = 0; phase < 4u; phase++) {
        char name[48];
        snprintf(name, sizeof name, "floor_transition_phase_%u", phase);
        add_case_civic(name, &world, 7u, 0,
                       DUEL_CIVIC_PACK(DUEL_CIVIC_FLOOR_WORKSHOP, DUEL_CIVIC_MODE_NORMAL, 0),
                       INCANTATION_FLOOR_TRANSITION_PACK(DUEL_CIVIC_FLOOR_COMMONS, phase, true));
    }

    for (uint8_t tier = 1; tier <= 4; tier++) {
        char name[48];
        snprintf(name, sizeof name, "ward_tier_%u", tier);
        sim_init(&world, SIMF_AUTHORITATIVE, 0);
        world.wiz[0].ward_strength = tier;
        world.wiz[0].ward_focus = tier - 1u;
    ...
    sim_init(&world, SIMF_AUTHORITATIVE, 0);
    world.wiz[0].stance = DUEL_STANCE_MEDITATE;
    world.wiz[0].hp = 3;
    world.wiz[0].ward_strength = 2;
    world.wiz[0].ward_focus = 2;
    world.wiz[1].stance = DUEL_STANCE_MEDITATE;
    world.wiz[1].hp = 4;
    ...
    sim_init(&world, SIMF_AUTHORITATIVE, 0);
    world.wiz[0].stance = DUEL_STANCE_FORTIFY;
    world.wiz[0].ward_strength = 1;
    world.wiz[1].stance = DUEL_STANCE_FORTIFY;
    world.wiz[1].ward_strength = 1;
    ...
    sim_init(&world, SIMF_AUTHORITATIVE, 0);
    world.wiz[0].inc_state = INC_COLLECTING;
    world.wiz[0].ward_strength = 1;
    world.wiz[0].inc.key_count = 3;
    world.wiz[0].inc.seen_pos = 7;
    add_case("collecting_short", &world, 3, 0);
    world.wiz[0].inc.key_count = 64;
    world.wiz[0].inc.seen_pos = 0xffffu;
    world.wiz[0].inc.turns = 16;
    world.wiz[0].inc.layer_transitions = 8;
    world.wiz[0].inc.overlap_peak = 5;
    world.wiz[0].inc.rhythm_changes = 8;
    world.wiz[0].ward_strength = 4;
    add_case("collecting_saturated", &world, 9, 0);

    sim_init(&world, SIMF_AUTHORITATIVE, 0);
    world.wiz[0].inc_state = INC_WINDUP;
    world.wiz[0].cast_windup = 6;
    world.wiz[0].windup_total = 12;
    world.wiz[0].ward_strength = 3;
    world.wiz[0].pending_desc = descriptor(SPELL_BEAM, 3);
    add_case("windup_beam", &world, 5, 0);
    world.wiz[0].inc_state = INC_PREPARED;
    world.wiz[0].prepared = 1;
    world.wiz[0].prepared_desc = world.wiz[0].pending_desc;
        ...
        for (uint8_t variant = 0; variant < 4; variant++) {
            for (uint8_t form = 0; form <= SPELL_CONJURE; form++) {
                ...
                world.spell[side].kind = DUEL_KIND_WITH_TIER(
                    DUEL_KIND_PACK(SPELL_DESC_ELEMENT(world.spell[side].descriptor), MOD_NONE,
                                   PAY_IMPACT),
                    variant);
                world.spell[side].progress =
                    form == SPELL_BEAM          ? 128u
                    : form == SPELL_SINGULARITY ? 144u
                    : form == SPELL_SWARM       ? (uint8_t)(((3u + variant) << 5) | 14u)
                    : form == SPELL_CHAIN       ? 160u
                    : form == SPELL_CONJURE     ? (uint8_t)(((2u + variant) << 5) | 14u)
                                                : 72u;
    ...
    struct temporal_case {
        const char *name;
        uint8_t form, progress, trajectory, tempo, trend;
    } temporal[] = {
        ...
        {"swarm_gather", SPELL_SWARM, (uint8_t)((6u << 5) | 5u), TRAJ_MID, TEMPO_FRANTIC,
         TREND_ACCELERATING},
        {"swarm_launch", SPELL_SWARM, (uint8_t)((5u << 5) | 14u), TRAJ_MID, TEMPO_FRANTIC,
         TREND_ACCELERATING},
    ...
    for (size_t i = 0; i < sizeof temporal / sizeof temporal[0]; i++) {
        sim_init(&world, SIMF_AUTHORITATIVE, 0);
        uint32_t desc = SPELL_DESC_PACK(
            temporal[i].form, ELEM_FORCE,
            !strncmp(temporal[i].name, "return", 6) ? PAY_HEAL : PAY_DAMAGE, temporal[i].trajectory,
            3, STATUS_NONE, INTERACT_SOLID, temporal[i].tempo, temporal[i].trend, 2);
        world.spell[0].active = 1;
        world.spell[0].descriptor = desc;
        world.spell[0].kind =
            DUEL_KIND_WITH_TIER(DUEL_KIND_PACK(ELEM_FORCE, MOD_NONE, PAY_IMPACT), 2);
    ...
    struct gap_family {
        const char *name;
        uint8_t form, element, trajectory, interaction;
    } gap[] = {
    ...
    for (size_t family = 0; family < sizeof gap / sizeof gap[0]; family++)
        for (uint8_t side = 0; side < 2u; side++)
            for (uint8_t stage = 0; stage < 3u; stage++) {
                char name[48];
                snprintf(name, sizeof name, "gap_%s_%c_%s", gap[family].name, side ? 'r' : 'l',
                         stage_name[stage]);
                sim_init(&world, SIMF_AUTHORITATIVE, 0);
                world.spell[side].active = 1u;
                world.spell[side].descriptor = SPELL_DESC_PACK(
                    gap[family].form, gap[family].element, PAY_DAMAGE, gap[family].trajectory, 2,
                    STATUS_NONE, gap[family].interaction, TEMPO_RAPID, TREND_STEADY, 1);
                world.spell[side].kind = DUEL_KIND_WITH_TIER(
...
int main(int argc, char **argv) {
    ...
    if (argc != 2) {
        fprintf(stderr, "usage: %s [--write-golden PATH | --dump-pgm DIR | PATH]\n", argv[0]);
        return 2;
    }
