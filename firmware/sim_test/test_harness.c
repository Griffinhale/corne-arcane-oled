...
void step(sim_world_t *w, uint32_t left, uint32_t right,
...
unsigned band_difference(const duel_fb_t *a, const duel_fb_t *b,
                                int y0, int y1) {
    unsigned n = 0;
    for (int y = y0; y <= y1; y++)
        for (int x = 0; x < DUEL_CANVAS_W; x++)
...
void incantation_render(duel_fb_t *fb, const duel_render_t *r, bool is_left,
                       bool diagnostics);
