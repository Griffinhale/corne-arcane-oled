...
static void event_px(duel_fb_t *fb, bool is_left, int x, int y) {
    duel_fb_px(fb, m13_desk_x(is_left, x), y, true);
}

static uint8_t event_action(uint8_t floor, uint8_t id) {
    switch (id) {
        case DUEL_M12_EVENT_RUNAWAY_SCROLL:
        case DUEL_M12_EVENT_DIPLOMATIC_COURIER:
            return DUEL_M12_ACTION_HANDLE_DELIVERY;
        case DUEL_M12_EVENT_JAMMED_GEAR:
            return floor == DUEL_M12_FLOOR_COMMONS ? DUEL_M12_ACTION_WATCH_ROOF
                                                   : DUEL_M12_ACTION_REACT;
        case DUEL_M12_EVENT_WORK_BREAK:
            return DUEL_M12_ACTION_REST;
        case DUEL_M12_EVENT_DAMAGE_COMPLAINT:
            return DUEL_M12_ACTION_INSPECT;
        default:
            return DUEL_M12_ACTION_WATCH_ROOF;
    }
}
...
static void draw_floor_event(duel_fb_t *fb, bool is_left, uint8_t floor,
    ...
    m13_point_t at = m13_occupation_anchor(floor, event_action(floor, id));
    int x = at.x, y = at.y;
    switch (id) {
        case DUEL_M12_EVENT_RUNAWAY_SCROLL: {
            ...
            if (end < 3) end = 3;
            m13_civic_hline(fb, is_left, end, x, y);
            m13_civic_vline(fb, is_left, x, y - 3, y + 1);
...
static void draw_shared_event(duel_fb_t *fb, bool is_left, uint8_t floor,
                              uint8_t id, uint8_t phase, bool quiet) {
    m13_point_t at = m13_occupation_anchor(floor, event_action(floor, id));
    if (id == DUEL_M12_EVENT_DIPLOMATIC_COURIER) {
        int top = at.y - 12;
        static const uint8_t reach[4] = {27, 31, 29, 26};
        m13_civic_vline(fb, is_left, at.x, top, at.y);
        m13_civic_hline(fb, is_left, at.x, reach[phase & 3u], top);
        m13_civic_hline(fb, is_left, at.x, reach[phase & 3u], top + 3);
        ...
        m13_civic_vline(fb, is_left, at.x - 1, at.y - 1, at.y);
    } else {
        ...
        for (int x = 0; x < DUEL_CANVAS_W; x++) {
            int dy = floor == DUEL_M12_FLOOR_COMMONS ? ((x / 4) & 1) :
                     floor == DUEL_M12_FLOOR_RESEARCH ? ((x / 3) & 1) : ((x / 2) & 1);
            duel_fb_px(fb, x, base - dy, true);
        }
        int streamers = phase == DUEL_M12_EVENT_PHASE_ACTIVE ? 3 :
                        phase == DUEL_M12_EVENT_PHASE_COOLDOWN ? 0 : 1;
        for (int i = 0; i < streamers; i++)
            m13_civic_vline(fb, is_left, (at.x + i * 7) & 31, base - 4, base - 1);
        if (!quiet && phase == DUEL_M12_EVENT_PHASE_ACTIVE)
...
void draw_rare_event(duel_fb_t *fb, const duel_render_t *r, bool is_left) {
    ...
    if (id == DUEL_M12_EVENT_NONE || id >= DUEL_M12_EVENT_COUNT) return;
    uint8_t target = DUEL_EVENT_TARGET(r->revision);
    if ((target == DUEL_M12_EVENT_TARGET_LEFT && !is_left) ||
        (target == DUEL_M12_EVENT_TARGET_RIGHT && is_left)) return;
