...
void duel_fb_clear(duel_fb_t *fb) { memset(fb->bits, 0, sizeof fb->bits); }

void duel_fb_px(duel_fb_t *fb, int x, int y, bool on) {
    if (x < 0 || x >= DUEL_CANVAS_W || y < 0 || y >= DUEL_CANVAS_H)
        return;
...
int duel_fb_desk_x(bool is_left, int x) { return is_left ? x : DUEL_CANVAS_W - 1 - x; }
